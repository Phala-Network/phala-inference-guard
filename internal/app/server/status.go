...
func (s *proxyServer) statusLogLine() string {
	...
	return fmt.Sprintf(
		"%s predictive={mode=%s attempts=%d fit=%d risk=%d unknown=%d reject=%d reservations=%d last=%s/%s pressure=%s capacity=%s/%s prefill=%s/%d kv=%d/%d/%d tps_proxy=%.3f sustained_tps=%.3f/%.3f reference=%.3f ready=%t sequences=%d/%d/%d unobserved=%d qos_budget=%t/%d router=%t/%s/%d observation=%t/%t/%d/%d compatibility=%d/%d/%d/%d}",
