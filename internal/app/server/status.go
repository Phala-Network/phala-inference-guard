...
func (s *proxyServer) statusLogLine() string {
	...
	return fmt.Sprintf(
		"%s predictive={mode=%s attempts=%d fit=%d risk=%d unknown=%d reject=%d reservations=%d action=%s reason=%s pressure=%s prefill=%s/%d kv=%d/%d/%d tps=%.3f/%.3f router=%t/%s/%d observer=%t/%t/%d/%d compatibility=%d/%d/%d/%d}",
		...
		input.RequestAwareAggregateTPSProxy, input.RequestAwareProjectedTPSProxy,
