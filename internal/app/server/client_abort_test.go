...
func TestCopyResponseCancelsUpstreamBodyWhenClientContextCancels(t *testing.T) {
	...
	go func() {
		errCh <- srv.copyResponseWithOptionalKeepAlive(ctx, httptest.NewRecorder(), response, true, time.Now(), semanticResponseCallbacks{})
	}()
...
func TestStreamingResponseDisconnectDoesNotCountProxyCopyError(t *testing.T) {
	...
	result := srv.proxyStreamingRequest(srv.backends[0], writer, request, false, time.Now(), semanticResponseCallbacks{})
