...
func (g contextGate) evaluate(work predictive.RequestWork) gateDecision {
	hardInput := work.Estimate.MaximumSequenceKVReservationInputTokens
	if hardInput <= 0 || hardInput > g.maximumInputTokens ||
		work.Estimate.DecodeHorizonTokens > g.maxModelLenTokens-hardInput {
...
func (g prefillGate) computeTokens(state ProjectedState, selectionTokens int64) int64 {
	...
	if availableCredit <= 0 {
		computed = selectionTokens
	} else if requestedCredit > availableCredit {
		computed = selectionTokens - availableCredit
	}
...
func (g prefillGate) evaluate(state ProjectedState, work predictive.RequestWork) (gateDecision, PrefillClass, int64) {
	class := g.classify(work.PrefillInputTokens)
