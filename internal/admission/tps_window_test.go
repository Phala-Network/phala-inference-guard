...
func TestV01215CompletionBetweenPollsCannotFabricateCurrentTPSEvidence(t *testing.T) {
	...
	decision := (tpsGate{}).evaluate(ProjectedState{RawRunning: 1, TPS: snapshot}, DefaultWindowConcurrency, RequestPriorityBasic)
...
func TestV01223TPSWindowHealthUsesRollingToleranceAndCurrentRecovery(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			decision := (tpsGate{}).evaluate(ProjectedState{TPS: snapshot}, DefaultWindowConcurrency, RequestPriorityBasic)
