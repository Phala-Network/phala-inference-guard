...
func TestParseSampleScopesCapabilityCountersByModelAndSource(t *testing.T) {
	sample := ParseSample(`
vllm:num_requests_running{model_name="served",engine="0"} 0
vllm:num_requests_waiting{model_name="served",engine="0"} 0
vllm:num_preemptions_total{model_name="served",engine="0"} 0
vllm:generation_tokens_total{model_name="served",engine="0"} 0
vllm:prompt_tokens_by_source_total{model_name="served",engine="0",source="local_compute"} 100
vllm:prompt_tokens_by_source_total{model_name="served",engine="1",source="local_compute"} 25
vllm:prompt_tokens_by_source_total{model_name="served",engine="0",source="local_cache_hit"} 7
vllm:prompt_tokens_by_source_total{model_name="other",engine="0",source="local_compute"} 9999
vllm:request_prefill_time_seconds_count{model_name="served",engine="0"} 2
vllm:request_prefill_time_seconds_count{model_name="served",engine="1"} 1
vllm:request_prefill_time_seconds_sum{model_name="served",engine="0"} 0.75
vllm:request_prefill_time_seconds_sum{model_name="served",engine="1"} 0.25
vllm:request_prefill_time_seconds_sum{model_name="other",engine="0"} 99
`)
	if !sample.PromptLocalComputeOK || sample.PromptLocalCompute != 125 ||
		!sample.PromptLocalCacheHitOK || sample.PromptLocalCacheHit != 7 ||
		!sample.PrefillMetricsOK || sample.PrefillRequests != 3 || sample.PrefillSeconds != 1 {
		t.Fatalf("capability counters = %#v", sample)
	}
}

func TestParseSampleRejectsIncompleteOrNonFiniteCapabilityCounters(t *testing.T) {
	sample := ParseSample(`
vllm:num_requests_running{model_name="served",engine="0"} 0
vllm:num_requests_waiting{model_name="served",engine="0"} 0
vllm:num_preemptions_total{model_name="served",engine="0"} 0
vllm:generation_tokens_total{model_name="served",engine="0"} 0
vllm:prompt_tokens_by_source_total{model_name="served",engine="0",source="local_compute"} +Inf
vllm:request_prefill_time_seconds_count{model_name="served",engine="0"} 1
`)
	if sample.PromptLocalComputeOK || sample.PrefillMetricsOK {
		t.Fatalf("invalid capability counters did not fail closed: %#v", sample)
	}
}
